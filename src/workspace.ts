import { chmodSync, copyFileSync, cpSync, existsSync, lstatSync, mkdirSync, readlinkSync, rmSync, symlinkSync, writeFileSync } from 'node:fs';
 ...
 */
export function initWorkspace(cfg: PepperConfig, configPath?: string): WorkspaceStatus {
  let created = false;
  ...
  if (configPath) ensurePepperctlShim(cfg.workspacePath, configPath);
...
/**
 * The skill and AGENTS.md tell the agent to run `pepperctl` — this shim is
 * what makes that a real command in the sandbox, with the right entrypoint and
 * config for THIS deployment baked in. Regenerated at every startup; the
 * adapter puts workspace/tools on the agent's PATH.
 */
export function ensurePepperctlShim(
  ws: string,
  configPath: string,
  entry: string = join(here, 'pepperctl.js'),
): boolean {
  if (!existsSync(entry)) {
    logger.warn({ entry }, 'pepperctl entrypoint not found — shim not written (dev mode?)');
    return false;
  }
  const toolsDir = join(ws, 'tools');
  mkdirSync(toolsDir, { recursive: true });
  const body = `#!/usr/bin/env bash
# Generated by pepperd at startup — do not edit; changes are overwritten.
export PEPPER_CONFIG="\${PEPPER_CONFIG:-${configPath}}"
exec "${process.execPath}" "${entry}" "$@"
`;
  writeFileSync(join(toolsDir, 'pepperctl'), body, { mode: 0o755 });
  return true;
}
